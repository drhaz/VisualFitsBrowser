...
// $Id: VRangeSlider.java,v 1.1.1.1 2009/02/17 22:24:29 abrighto Exp $
...
public class RangeSlider extends JComponent implements MouseListener,
    ...
    private static final int SLIDER_PREF_HEIGHT = 55;

    /** Preferred slider width. */
    private static final int SLIDER_PREF_WIDTH = 300;

    /** Width of grip. */
    private static final int GRIP_WIDTH = 20;

    /** Height of grip. */
    private static final int GRIP_HEIGHT =20;
    ...
    private static final int SLIDER_LINE_HEIGHT = GRIP_HEIGHT + 1;

    /** Width of slider line. */
    private static final int SLIDER_LINE_WIDTH = 3;

    /** Height of font. */
    private static final int FONT_HEIGHT = 20;

    /** Y-coordinate of top of font. */
    private static final int FONT_TOP_Y = 30;
    ...
    private Color arrowColor = Color.orange;

    /** list of listeners for change events */
    protected EventListenerList listenerList = new EventListenerList ();

    /** Internal numberformatter for the selected values */

    protected NumberFormat myNF = null;

    public NumberFormat getMyNF () {
        return myNF;
    }

    public void setMyNF (NumberFormat myNF) {
        this.myNF = myNF;
    }
	...
	name = label;
	myNF = NumberFormat.getIntegerInstance ();
     ...
     */
    private static boolean containedIn (int px, int py, int x, int y, int w,
	    int h) {
	...
	if (containedIn (x, y, minGrip - (GRIP_WIDTH - 1), GRIP_TOP_Y,
	    ...
	    minSlide = true;
	} else if (containedIn (x, y, maxGrip, GRIP_TOP_Y, GRIP_WIDTH,
		GRIP_HEIGHT)) {
	    // mouse pressed in right grip
	    maxSlide = true;
	} else if (containedIn (x, y, minGrip, GRIP_TOP_Y - 3, maxGrip
		- minGrip, GRIP_TOP_Y + SLIDER_LINE_HEIGHT - 1)) {
    ...
    /**
     * register to receive change events from this object whenever the min or
     * max values are changed.
     */
	...
	return (((maxLimit - minLimit) * ((double) (pos - GRIP_WIDTH))) / (double) (width - (GRIP_WIDTH * 2)))
		+ minLimit;
    }

    private int valueToGrip (double value, int width) {
	double rdouble = (((value - (double) minLimit) * (double) (width - (GRIP_WIDTH * 2))) / (maxLimit - minLimit));
    ...
    /**
     * Called whenever the min or max value is updated. This method is meant to
     * be overridden by extension classes.
     */
	...
	lines[1] = new Line2D.Double (0, GRIP_TOP_Y - 4, 0, GRIP_TOP_Y
		+ SLIDER_LINE_HEIGHT);

	lines[2] = new Line2D.Double (0, GRIP_TOP_Y - 4, SLIDER_LINE_WIDTH,
		GRIP_TOP_Y - 4);

	lines[3] = new Line2D.Double (0, GRIP_TOP_Y + SLIDER_LINE_HEIGHT,
		SLIDER_LINE_WIDTH, GRIP_TOP_Y + SLIDER_LINE_HEIGHT);

	lines[4] = new Line2D.Double (right, GRIP_TOP_Y - 4, right, GRIP_TOP_Y
		+ SLIDER_LINE_HEIGHT);

	lines[5] = new Line2D.Double (right, GRIP_TOP_Y - 4, right
		- SLIDER_LINE_WIDTH, GRIP_TOP_Y - 4);
	...
	if (lSlideMoved) {
	    g2.setColor (bg);
	    g2.fillRect (SLIDER_LINE_WIDTH, GRIP_TOP_Y, maxGrip - 3,
		    GRIP_HEIGHT);

	    g2.setColor (fg);
	    g2.drawLine (SLIDER_LINE_WIDTH, GRIP_MIDDLE_Y, maxGrip - 3,
		    GRIP_MIDDLE_Y);

	    g2.setStroke (shadowStroke);
	    g2.setColor (Color.white);
	    g2.drawLine (SLIDER_LINE_WIDTH, GRIP_MIDDLE_Y + 1, maxGrip - 3,
		    GRIP_MIDDLE_Y + 1);
	...
	if (rSlideMoved) {
	    ...
	    g2.setStroke (shadowStroke);
	    g2.setColor (Color.white);
	    g2.drawLine (minGrip + 1, GRIP_MIDDLE_Y + 1, w - 3,
		    GRIP_MIDDLE_Y + 1);
	    g2.setStroke (defaultStroke);
	...
	if (lSlideMoved || rSlideMoved) {
	    ...
	    g2.draw3DRect (minGrip + 1, GRIP_MIDDLE_Y - 4, maxGrip - minGrip
		    - 1, 9, true);
	    g2.fill3DRect (minGrip + 1, GRIP_MIDDLE_Y - 4, maxGrip - minGrip
		    - 1, 9, true);
	}
	if (textChanged)
	    drawLabels (g2, w);
	...
	int w = getSize ().width;
	g2.setFont(GUIConsts.InformationFont);
	FontMetrics fm = g2.getFontMetrics ();
	if (lastMinLimit != minLimit || lastW != w) {
	    // minimum bound text string
	    g2.setColor (getBackground ());
	    int sw = fm.stringWidth ("" + myNF.format (lastMinLimit));
	...
	if (lastMaxLimit != maxLimit || lastW != w) {
	    ...
	    g2.setColor (getBackground ());
	    int sw = fm.stringWidth ("" + myNF.format (lastMaxLimit));
	    g2.fillRect (lastW - 4 - sw, FONT_TOP_Y, sw, FONT_HEIGHT);
	    lastMaxLimit = maxLimit;
	}
	String minS = myNF.format (minValue);
	String maxS = myNF.format (maxValue);
	...
	g2
		.drawString (curStr, (w - fm.stringWidth (curStr)) / 2,
			FONT_BOTTOM_Y);
    }
	...
	RangeSlider rs = new RangeSlider ("", 0.0, 100.0);
